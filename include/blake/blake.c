...
// outlen - The length of the output. Ranges from 0 to 64;
// out - Pointer value where the output will be written to. Cannot be NULL.
void blake2bHash(char* message, size_t mlen, uint8_t outlen, unsigned char *out[outlen]) {
    ...
    memset(final_chunk + (mlen - compressed), 0x0, 128 - (mlen - compressed));
